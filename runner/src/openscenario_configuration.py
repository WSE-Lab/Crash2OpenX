...
class OpenScenarioConfiguration(ScenarioConfiguration):
    ...
    def __init__(self, xml_tree, client, custom_params, filename=None):

        super(OpenScenarioConfiguration, self).__init__()

        self.xml_tree = xml_tree
        self.filename = os.path.abspath(filename) if filename else os.getcwd()
    ...
    def _validate_openscenario_configuration(self):
        ...
        xsd_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), "../scenario_runner/srunner/openscenario/OpenSCENARIO.xsd")
    ...
    def _prepare_opendrive_for_carla(opendrive_data, opendrive_path):
        try:
            ...
            raise AttributeError("The provided OpenDRIVE file is not valid XML: {} ({})".format(opendrive_path, exc))
        ...
        for index, road in enumerate(roads):
            ...
            if length <= 0.0:
                raise AttributeError("OpenDRIVE road {} has no positive length in {}".format(
        ...
        if missing_geometry:
            raise AttributeError("OpenDRIVE roads have no planView geometry in {}: {}".format(
        ...
        repaired_data, repaired_topology = repair_opendrive_xml(ET.tostring(root, encoding='unicode'))
        if repaired_topology:
            print("Patched {} OpenDRIVE junction/lane topology entries in memory for CARLA import".format(
                repaired_topology))
    ...
    def _set_carla_town(self):
        ...
        if world is None or (wmap is not None and wmap.name.split('/')[-1] != self.town):
            if ".xodr" in self.town:
                ...
                if index == -1:
                    raise AttributeError("The provided RoadNetwork '{}' does not contain an <OpenDRIVE> root".format(self.town))
                data = data[index:]
                data = self._prepare_opendrive_for_carla(data, self.town)

                old_map = ""
                if wmap is not None:
                    old_map = wmap.to_opendrive()
                    index = old_map.find('<OpenDRIVE')
                    old_map = old_map[index:] if index != -1 else old_map
    ...
    def _get_actor_transform(self, actor_name):
        ...
        if not actor_found:
            for entity_action in self.storyboard.iter("EntityAction"):
                if entity_action.attrib.get('entityRef') != actor_name:
                    continue
                add_entity_action = entity_action.find('AddEntityAction')
                if add_entity_action is None:
                    continue
                position = add_entity_action.find('Position')
                if position is None:
                    continue
                ...
                actor_found = True
                self.logger.info(
                    " Actor '%s' uses its first AddEntityAction position as the initial spawn pose",
                    actor_name)
                break
