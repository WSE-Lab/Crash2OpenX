...
# Real CARLA render recording. CARLA must be started separately with
# -RenderOffScreen, not -nullrhi. Do not enable demo --no-rendering here.
export REQUIRE_EXISTING_CARLA="${REQUIRE_EXISTING_CARLA:-1}"
export USE_EXISTING_CARLA="${USE_EXISTING_CARLA:-1}"
...
export WAIT_TIMEOUT="${WAIT_TIMEOUT:-30}"
